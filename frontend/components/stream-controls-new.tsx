...
export function StreamControls() {
  ...
  useEffect(() => {
    const fetchStreamStatus = async () => {
      try {
        const res = await fetch("/api/live-stream/current-stream", {
          headers: {
  ...
  const handleStartStream = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setIsLive(true)
        alert("Stream started! RTMP URL: " + data.rtmp_url)
      } else {
        ...
        {isLive && streamStatus && (
          <div className="pt-4 border-t border-border space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium">{streamStatus.duration || "N/A"}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Current Viewers</span>
              <span className="font-medium">{streamStatus.viewerCount}</span>
            </div>
            <div className="flex justify-between text-sm">
